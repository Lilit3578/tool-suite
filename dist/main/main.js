...
function createLogger(scope = "App") {
  return {
    ...
    debug: (...args) => console.debug ? console.debug(`[${scope}]`, ...args) : console.log(`[${scope}]`, ...args)
  };
}
...
function getWindowConfig(widgetId, widgetWindowOptions) {
...
function getComponentType(widgetId) {
  const registryEntry = WINDOW_REGISTRY[widgetId];
  return registryEntry?.componentType || "palette";
}

// src/main/window-factory.ts
var logger = createLogger("WindowFactory");
async function createWindow(options) {
  ...
  const win = new import_electron.BrowserWindow({
    ...
    y: windowY,
    show: false,
    // CRITICAL: Don't show until ready
    ...
    webPreferences: {
      preload: import_path.default.join(__dirname, "../preload/preload.js"),
      contextIsolation: true,
      nodeIntegration: false
    }
  });
  const url = process.env.ELECTRON_RENDERER_URL ?? `file://${import_path.default.join(__dirname, "../renderer/index.html")}`;
...
var logger2 = createLogger("WidgetManager");
var WidgetManager = class {
  ...
  registerWidget(widget) {
    ...
    logger2.info(`Registered widget: ${widget.id}`);
  }
  getWidget(id) {
  ...
  async openWidgetWindow(id, payload) {
    const widget = this.widgets.get(id);
    if (!widget)
      throw new Error(`Widget ${id} not registered`);
    if (this.windows.has(id)) {
      const win2 = this.windows.get(id);
      if (!win2.isDestroyed()) {
        win2.show();
        win2.focus();
        win2.webContents.send("component-init", {
...
var Conf = class {
  ...
  */
  _closeWatcher() {
    ...
    if (this.#watchFile) {
      import_node_fs2.default.unwatchFile(this.path);
  ...
  _watch() {
    ...
    if (import_node_process6.default.platform === "win32" || import_node_process6.default.platform === "darwin") {
      ...
      import_node_fs2.default.watchFile(this.path, { persistent: false }, (_current, _previous) => {
        if (typeof this.#debouncedChangeHandler === "function") {
          this.#debouncedChangeHandler();
        }
      });
...
// node_modules/electron-store/index.js
var { app, ipcMain, shell } = import_electron2.default;
...
var ElectronStore = class extends Conf {
  constructor(options) {
    ...
    if (import_node_process7.default.type === "renderer") {
      const appData = import_electron2.default.ipcRenderer.sendSync("electron-store-get-data");
      if (!appData) {
        throw new Error("Electron Store: You need to call `.initRenderer()` from the main process.");
      }
...
var logger3 = createLogger("Settings");
...
var settingsManager = {
  ...
  incrementUsage(key) {
    ...
    logger3.debug(`usage ${key} -> ${usage[key]}`);
  }
};

// src/main/ipc-handlers.ts
var logger4 = createLogger("IPC");
function registerIpcHandlers(widgetManager2, callbacks) {
  import_electron3.ipcMain.handle("get-widgets", () => {
    return widgetManager2.getAllWidgets().map((w) => ({
      id: w.id,
      label: w.label,
      icon: w.icon,
  ...
  import_electron3.ipcMain.handle("open-widget", async (_, id, payload) => {
    logger4.info("open-widget", id, payload);
    try {
      if (id === "translator" && callbacks?.openTranslatorWidget) {
        const selectedText = payload?.selectedText || callbacks.getCapturedText();
        logger4.info("Opening translator widget with text:", selectedText);
        await callbacks.openTranslatorWidget(selectedText);
        return { success: true };
      }
      await widgetManager2.openWidgetWindow(id, payload);
      return { success: true };
    } catch (error) {
      logger4.error("Error opening widget:", error);
      return { success: false, error: String(error) };
    }
  });
  import_electron3.ipcMain.handle("get-captured-text", () => {
    return callbacks?.getCapturedText() || "";
  });
  import_electron3.ipcMain.handle("show-action-popover", async (_, resultText, position) => {
    logger4.info("=== IPC show-action-popover handler called ===");
    logger4.info("resultText:", resultText);
    logger4.info("position:", position);
    logger4.info("callbacks?.showActionPopover available:", !!callbacks?.showActionPopover);
    if (callbacks?.showActionPopover) {
      try {
        await callbacks.showActionPopover(resultText, position);
        logger4.info("showActionPopover callback completed successfully");
        return { success: true };
      } catch (error) {
        logger4.error("Error in showActionPopover callback:", error);
        return { success: false, error: String(error) };
      }
    }
    logger4.warn("showActionPopover callback not available");
    return { success: false, error: "Invalid parameters" };
  });
  import_electron3.ipcMain.handle("execute-action", async (_, actionId, selectedText) => {
    logger4.info("execute-action", actionId, "with text:", selectedText);
    try {
      for (const w of widgetManager2.getAllWidgets()) {
        const a = w.actions?.find((x) => x.id === actionId);
        if (a && typeof a.handler === "function") {
          logger4.info("Found action handler for:", actionId);
          settingsManager.incrementUsage(actionId);
          try {
            const result = await a.handler(selectedText);
            logger4.info("Action result:", result);
            return { success: true, result };
          } catch (err) {
            logger4.error("action handler error", err);
            return { success: false, error: String(err) };
          }
        }
      }
      logger4.warn("Action not found:", actionId);
      return { success: false, error: "Action not found" };
    } catch (error) {
      logger4.error("Error executing action:", error);
      return { success: false, error: String(error) };
    }
  });
  import_electron3.ipcMain.handle("get-suggestions", (_, query) => {
    const widgets = widgetManager2.getAllWidgets();
    const items = [];
    widgets.forEach((w) => {
...
var logger5 = createLogger("TextTranslator");
var TextTranslator = class {
  constructor(manager) {
    this.manager = manager;
    this.id = "translator";
    this.label = "Translator";
    this.icon = "globe";
    this.componentType = "translator";
    this.windowOptions = {
      width: 480,
      height: 540,
      transparent: false,
    ...
    this.actions = [
      { id: "translate-it", label: "Translate to Italian", handler: (text) => this.translate(text, "it") },
      { id: "translate-en", label: "Translate to English", handler: (text) => this.translate(text, "en") },
      { id: "translate-fr", label: "Translate to French", handler: (text) => this.translate(text, "fr") },
      { id: "translate-de", label: "Translate to German", handler: (text) => this.translate(text, "de") },
      { id: "translate-es", label: "Translate to Spanish", handler: (text) => this.translate(text, "es") }
      // extend list...
    ];
  }
  async initialize() {
  ...
  async translate(text, target = "en") {
    ...
    try {
      ...
      logger5.error("translate error", err);
      return { success: false, error: String(err) };
    }
  }
};
...
var logger6 = createLogger("TextCapture");
async function captureSelectedText() {
  if (process.platform !== "darwin") {
    logger6.warn("Text capture only supported on macOS");
    return "";
  }
  try {
...
return the clipboard as string`;
    ...
    const capturedText2 = stdout.trim();
    logger6.info(`Captured text: "${capturedText2.substring(0, 50)}${capturedText2.length > 50 ? "..." : ""}"`);
    return capturedText2;
  } catch (error) {
    logger6.error("Failed to capture selected text", error);
    return "";
  }
}
  ...
  mainWindow.on("blur", () => {
    ...
    blurTimeout = setTimeout(() => {
      ...
      if (actionPopoverWindow && !actionPopoverWindow.isDestroyed()) {
        logger7.info("Hiding action popover window");
...
async function createTranslatorWindow(selectedText) {
  const widget = widgetManager.getWidget("translator");
  if (translatorWindow && !translatorWindow.isDestroyed()) {
    const { x: x2, y: y2 } = import_electron4.screen.getCursorScreenPoint();
    translatorWindow.setPosition(x2, y2, false);
  ...
  const { x, y } = import_electron4.screen.getCursorScreenPoint();
...
async function createActionPopoverWindow(resultText, position) {
  logger7.info("createActionPopoverWindow called");
  logger7.info("resultText:", resultText, "type:", typeof resultText);
  logger7.info("position:", position);
  ...
  logger7.info("Normalized text:", text);
  const pos = {
    x: Math.round(position.x),
    y: Math.round(position.y)
  };
  if (actionPopoverWindow && !actionPopoverWindow.isDestroyed()) {
    logger7.info("Reusing existing action popover window");
    ...
    logger7.info("Action popover window updated and shown at:", pos);
    return actionPopoverWindow;
  }
  logger7.info("Creating new action popover window");
  ...
  logger7.info("Action popover window created at:", pos);
  return actionPopoverWindow;
}
function createTray() {
  const size = 16;
  ...
  const image = import_electron4.nativeImage.createFromDataURL(dataUrl);
  image.setTemplateImage(true);
  tray = new import_electron4.Tray(image);
...
import_electron4.app.on("ready", async () => {
  ...
  registerIpcHandlers(widgetManager, {
    ...
    openTranslatorWidget: async (selectedText) => {
      logger7.info("openTranslatorWidget called with text:", selectedText || capturedText);
      try {
        if (mainWindow && !mainWindow.isDestroyed()) {
          mainWindow.removeAllListeners("blur");
          logger7.info("Removed main window blur handler");
        }
        const win = await createTranslatorWindow(selectedText || capturedText);
        logger7.info("Translator window created/shown");
        if (mainWindow && !mainWindow.isDestroyed()) {
          mainWindow.hide();
          logger7.info("Main window hidden after translator opened");
        }
        return win;
      } catch (error) {
        logger7.error("Error creating translator window:", error);
        throw error;
      }
    },
    showActionPopover: async (resultText, relativePosition) => {
      logger7.info("=== showActionPopover CALLED ===");
      logger7.info("resultText:", resultText);
      logger7.info("relativePosition:", relativePosition);
      try {
        let screenX = relativePosition.x;
        let screenY = relativePosition.y;
        if (mainWindow && !mainWindow.isDestroyed() && mainWindow.isVisible()) {
          const [mainX, mainY] = mainWindow.getPosition();
          logger7.info("Palette position:", { mainX, mainY });
          screenX = Math.round(mainX + relativePosition.x);
          screenY = Math.round(mainY + relativePosition.y);
          logger7.info("Screen position:", { screenX, screenY });
        } else {
          logger7.warn("Palette window not available, using cursor position");
          const { x, y } = import_electron4.screen.getCursorScreenPoint();
          screenX = x + 20;
          screenY = y;
        }
        const win = await createActionPopoverWindow(resultText, { x: screenX, y: screenY });
        logger7.info("=== showActionPopover SUCCESS ===");
        return win;
      } catch (error) {
        logger7.error("=== showActionPopover ERROR ===", error);
        throw error;
      }
    }
  });
  ...
  import_electron4.globalShortcut.register(shortcut, async () => {
    logger7.info("Global shortcut pressed");
    capturedText = await captureSelectedText();
    logger7.info(`Captured text: "${capturedText}"`);
    const { x, y } = import_electron4.screen.getCursorScreenPoint();
  ...
  import_electron4.app.on("will-quit", () => import_electron4.globalShortcut.unregisterAll());
});
...
import_electron4.app.on("activate", async () => {
